...
Next.js(클라이언트)와 Spring Boot(서버) 기반의 ESG 서비스에서 **인증·데이터 흐름의 틀을** 구축했습니다.
**세션(Session) 방식 인증**으로 로그인/회원가입 전 과정을 설계·구현하고, **DART API**를 통해 기업 정보를 수집·매핑했습니다.
사용자와 기업 간 **다대다(M:N) 관심기업 엔티티** 및 관련 API/화면을 구현해, 개인화된 관심기업 관리 기능을 완성했습니다.

## 개선 방향

프로젝트를 다시 제작한다면 **세션 기반 인증 대신 JWT(Json Web Token) 기반 인증 방식을 도입**할 것입니다.
세션 방식은 서버가 로그인 상태를 메모리에 저장하는 **상태 기반(stateful)** 구조로, 단일 서버 환경에서는 간단하고 효율적이지만
서버 확장이나 로드 밸런싱 환경에서는 세션 동기화 이슈가 발생할 수 있습니다.

반면 JWT는 클라이언트가 인증 토큰을 직접 보유하는 **무상태(stateless)** 구조로,
각 요청 시 헤더에 토큰을 포함해 인증을 수행하므로 **분산 서버 환경 및 클라우드 아키텍처에 적합**합니다.
또한 Spring Security의 OncePerRequestFilter를 활용하여 JWT 인증 필터를 구성하면,
LoginCheckInterceptor를 대체하면서도 보다 확장성과 보안성을 갖춘 인증 구조를 설계할 수 있습니다.

아울러 프로젝트의 규모가 커지면서 파일 수가 증가한다면,
기존의 단순한 **계층형 구조(Layered Architecture)** 대신 **도메인 기반 구조(DDD, Domain-Driven Design)** 로 재구성하는 것이 더 효율적이라고 생각합니다.
이를 통해 각 도메인이 독립적인 단위로 관리되어 유지보수성과 확장성이 크게 향상될 것입니다.
